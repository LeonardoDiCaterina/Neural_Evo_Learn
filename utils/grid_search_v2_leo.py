...
def montecarlo_cv(
    ...
    y,
    params: dict,
    n_splits: int = 3, #magic statistician number :)
    test_size: float = 0.2,
    seed: int = 1,
    ...
    for i in range(n_splits):
        ...
        X_val_prep, y_test_prep = preprocess_data(X_test, y_test)
        
        model = model_class(X_train = X_train_prep,
        ...
        #score = scorer(model,X_test, y_test) if scorer else model.score(X_test, y_test)
        #scores.append(score)
    return 0 
